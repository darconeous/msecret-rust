...
* Output
    * A pseudo-random prime of exactly `bit_length` bits.
...
3. Usability check: If `gcd(E, candidate_prime-1) ≠ 1`, goto step 1.
...
make against P and Q is ensuring that `gcd(E, P-1) = 1`
and `gcd(E, Q-1) = 1`, which is required for RSA to work
at all.

The private exponent `D` is calculated as
`D = E^(-1) mod λ(N)`, where `λ(N) = lcm(P-1, Q-1)` is
Carmichael's totient function, as recommended by
PKCS#1 v2.2. This yields the smallest valid private
exponent for the given key.
...
2. Derive a pseudo-random integer between 0 and `order` using the algorithm
   described above and assign that value to `out`.
3. Assert that `out` is not zero and not equal to `order`. (Either case is,
   of course, extremely unlikely — probability ~2/2^256 for standard curves.)
